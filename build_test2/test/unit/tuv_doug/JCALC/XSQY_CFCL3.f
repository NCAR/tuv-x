      subroutine XSQY_CFCL3(nw,wl,wc,nz,tlev,airlev,j,sq,jlabel,pn)
!-----------------------------------------------------------------------------!
!   purpose:                                                                  !
!   provide product (cross section) x (quantum yield) for cfcl3 photolysis:   !
!          CFCl3 + hv -> 3Cl                                                  !
...
!---------------------------------------------------
      ...
      do iw = 1, nw-1
        ...
        IF ((wc(iw) .GE. 174.) .AND. (wc(iw) .LE.230.)) THEN
     ...
     &                    + (300.0-273.0)*BB(nloop)*wc(iw)**lp(nloop)
      ...
      enddo
!---------------------------------------------------
!     ... For wavelengths >230 nm and <174 nm
!---------------------------------------------------
      open(kin,file=TRIM(pn)//'XS_CFCL3_JPL06.txt',status='old')
