...
namespace tuvx
{
  ...
  inline double ComputeEffectiveCosine(
      bool   nid_has_value,
      double delta_od,
      double delta_slant,
      double large_sentinel)
  {
      ...
      for (std::size_t wl = 0; wl < N_WAVELENGTHS; ++wl)
      {
        ...
        if (COSINE_SOLAR_ZENITH < 0.0)
        {
          slant_optical_depths[0] = SlantOpticalDepth(
              0, geom.nid_[0], geom.dsdh_[0], scaled_optical_depth);
        }
        ...
        for (std::size_t i = 0; i < N_LAYERS; ++i)
        {
          ...
          slant_optical_depths[i + 1] = SlantOpticalDepth(
              i + 1, geom.nid_[i + 1], geom.dsdh_[i + 1], scaled_optical_depth);
          ...
          e1[i] = COEFF.e1_; e2[i] = COEFF.e2_; e3[i] = COEFF.e3_; e4[i] = COEFF.e4_;
