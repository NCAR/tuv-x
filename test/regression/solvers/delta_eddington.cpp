...
void CheckInputs(
    const std::vector<double>& solar_zenith_angles,
    ...
    const tuvx::RadiatorState<tuvx::Array3D<double>>& accumulated_radiator_states)
{
  ...
  ASSERT_NEAR(solar_zenith_angles.at(1), 28.199 * 3.14159265 / 180.0, 1.0e-5);
...
SolverOutput RunDeltaEddingtonSolver(const SolverInput input)
{
  ...
  grids["altitude [m]"] =
      CreateGrid<GridPolicy>("m", input.n_columns_, input.n_levels_, input.altitude_mid_points_, input.altitude_edges_);
  grids["wavelength [m]"] =
