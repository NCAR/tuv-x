      subroutine XSQY_HCFC141b(nw,wl,wc,nz,tlev,airlev,j,sq,jlabel,pn)
!-----------------------------------------------------------------------------!
!   purpose:                                                                  !
...
!---------------------------------------------------
      tin(:)   = tlev(:)
!---------------------------------------------------
!     ... jlabel(j) = 'hcfc141b -> 2cl'
!---------------------------------------------------
      j = j+1
      jlabel(j) = 'HCFC141b + hv -> 2Cl'

!---------------------------------------------------
!    Derive temperature dependence 
!---------------------------------------------------
! Temperature dependence good between 
!     210-300K and 172 nm-240 nm
!---------------------------------------------------
      iwc      = 1
      ytmp(:,:)= 0.0 
      ...
      do iw = 1, nw-1
        ...
        IF ((wc(iw) .GE. 172.) .AND. (wc(iw) .LE.240.)) THEN
...
!---------------------------------------------------
      open(kin,file=TRIM(pn)//'XS_HCFC141b_JPL10.txt',status='old')
