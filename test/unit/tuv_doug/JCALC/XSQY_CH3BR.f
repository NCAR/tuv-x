      subroutine XSQY_CH3BR(nw,wl,wc,nz,tlev,airlev,j,sq,jlabel,pn)
!-----------------------------------------------------------------------------!
!   purpose:                                                                  !
...
!----------------------------------------------
      j = j+1
      jlabel(j) = 'CH3Br + hv -> Br'

!----------------------------------------------------------
!    Derive temperature dependence 
!----------------------------------------------------------
! Temperature dependence good between 210-300K and 200-280nm
      iwc      = 1
      ytmp(:,:)= 0.0 

      do iw = 1, nw-1
      
        IF ((wc(iw) .GE. 200.) .AND. (wc(iw) .LE.280.)) THEN
...
!----------------------------------------------------------
!     ... For wavelengths >280nm and <200 nm
!----------------------------------------------------------
      open(kin,file=TRIM(pn)//'XS_CH3BR_JPL06.txt',status='old')
