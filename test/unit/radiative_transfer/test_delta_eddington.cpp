...
TEST(DeltaEddington, TransparentAtmosphereOverheadSun)
{
  ...
  for (std::size_t lev = 0; lev < N_LEVELS; ++lev)
  {
    ...
    EXPECT_NEAR(fix.radiation_field_.spectral_irradiance_.direct_(0, lev, 0), 1.0, TOLERANCE)
        << "direct irradiance at level " << lev;
    // No diffuse
    EXPECT_NEAR(fix.radiation_field_.spectral_irradiance_.upwelling_(0, lev, 0), 0.0, TOLERANCE)
        << "upwelling irradiance at level " << lev;
    EXPECT_NEAR(fix.radiation_field_.spectral_irradiance_.downwelling_(0, lev, 0), 0.0, TOLERANCE)
...
TEST(DeltaEddington, TransparentAtmosphereWithAlbedo)
{
  ...
  EXPECT_NEAR(fix.radiation_field_.spectral_irradiance_.direct_(0, 0, 0), 1.0, TOLERANCE);
}
...
TEST(DeltaEddington, OutputDimensions)
{
  ...
  EXPECT_EQ(fix.radiation_field_.spectral_irradiance_.direct_.Size1(), DeltaEddingtonFixture::N_WAVELENGTHS);
  EXPECT_EQ(fix.radiation_field_.spectral_irradiance_.direct_.Size2(), DeltaEddingtonFixture::N_LAYERS + 1);
  EXPECT_EQ(fix.radiation_field_.spectral_irradiance_.direct_.Size3(), DeltaEddingtonFixture::N_COLUMNS);
}
