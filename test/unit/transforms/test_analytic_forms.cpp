...
TEST(LogNormalBands, PeaksAtCenter)
{
  ...
  tuvx::log_normal_bands<>(
      { tuvx::LogNormalBand<>{ .amplitude_ = 5.0, .width_ = 100.0, .center_ = 300e-9 } })(state, w);
...
TEST(LogNormalBands, SumsMultipleBands)
{
  ...
  tuvx::Array3D<double> w(1, 1, 1);
  tuvx::log_normal_bands<>(
      { tuvx::LogNormalBand<>{ .amplitude_ = 2.0, .width_ = 50.0, .center_ = 300e-9 },
...
TEST(LogNormalBands, ZeroOutsideRegion)
{
  ...
  tuvx::log_normal_bands<>(
      { tuvx::LogNormalBand<>{ .amplitude_ = 1.0, .width_ = 1.0, .center_ = 300e-9 } },
      250e-9,
      550e-9)(state, w);
...
TEST(LogNormalBands, RegionBoundariesInclusive)
{
  ...
  tuvx::Array3D<double> w(2, 1, 1);
  tuvx::log_normal_bands<>(
      { tuvx::LogNormalBand<>{ .amplitude_ = 1.0, .width_ = 1.0, .center_ = 400e-9 } },
      250e-9,
      550e-9)(state, w);
...
TEST(LogNormalBands, ExtremeWidthStaysFiniteAndDecays)
{
  ...
  tuvx::Array3D<double> w(2, 1, 1);
  tuvx::log_normal_bands<>(
      { tuvx::LogNormalBand<>{ .amplitude_ = 1.0, .width_ = 1.0e12, .center_ = 300e-9 } })(state, w);
...
TEST(ExpPolynomial, EmptyCoeffsIsExpZero)
{
  ...
  auto state = StateWithWavelengths({ 300e-9 });
  tuvx::Array3D<double> w(1, 1, 1);
  tuvx::exp_polynomial<>({}, 1.0, 7.0)(state, w);
...
TEST(ExpPolynomial, ConstantPolynomial)
{
  ...
  tuvx::exp_polynomial<>({ 2.0 })(state, w);
...
TEST(ExpPolynomial, LinearWithWavelengthScale)
{
  ...
  auto state = StateWithWavelengths({ 300e-9 });
  tuvx::Array3D<double> w(1, 1, 1);
  tuvx::exp_polynomial<>({ 1.0, 2.0 }, 1.0e9, 1.0)(state, w);
...
TEST(ExpPolynomial, OutputScaleApplied)
{
  auto state = StateWithWavelengths({ 300e-9 });
  tuvx::Array3D<double> w(1, 1, 1);
  tuvx::exp_polynomial<>({ 0.0 }, 1.0, 1.0e-4)(state, w);
...
TEST(ExpPolynomial, HornerMatchesQuadratic)
{
  ...
  auto state = StateWithWavelengths({ 300e-9 });
  tuvx::Array3D<double> w(1, 1, 1);
  tuvx::exp_polynomial<>({ c0, c1, c2 }, 1.0e9, 1.0e-4, 270e-9, 330e-9)(state, w);
...
TEST(ExpPolynomial, NoOverflowOutsideRegion)
{
  ...
  tuvx::Array3D<double> w(3, 1, 1);
  tuvx::exp_polynomial<>({ 0.0, 1.0 }, 1.0e9, 1.0, 250e-9, 350e-9)(state, w);
...
TEST(ExpPolynomial, BroadcastsOverHeightAndColumn)
{
  ...
  tuvx::exp_polynomial<>({ 0.5 })(state, w);
