      subroutine XSQY_CFC113(nw,wl,wc,nz,tlev,airlev,j,sq,jlabel,pn)
!-----------------------------------------------------------------------------!
!   purpose:                                                                  !
...
!---------------------------------------------------
      ...
      do iw = 1, nw-1
        ...
        IF ((wc(iw) .GE. 182.) .AND. (wc(iw) .LE.230.)) THEN
     ...
     &                    + (300.0-273.0)*BB(nloop)*wc(iw)**lp(nloop)
      ...
      enddo
!---------------------------------------------------
!     ... For wavelengths >230nm and <182 nm
!---------------------------------------------------
      open(kin,file=TRIM(pn)//'XS_CFC113_JPL06.txt',status='old')
