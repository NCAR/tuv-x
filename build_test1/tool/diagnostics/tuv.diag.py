...
#--------------------------
   ...
   try:
      varold = fileold.read_reals(dtype=np.float32)
   except:
...
#--------------------------
   ...
   if( np.any(varnew == 0.) ):
      ...
      print(varnew)
      print(f"{variable}.old")
      print(varold)
      diff = varold
   else:
      diff = (varold/varnew - 1.)*100.
...
#  print(f"{variable} diff max indices = {maxind}")
   ...
   if( is2d ):
      ...
      print(f"{variable} @ max diff = {varold[maxind]}, {varnew[maxind]}")
      print(f"{variable} @ max diff = {varold[88]}, {varnew[88]}")
